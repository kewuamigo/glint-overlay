...
/// Bounded keyed-mutex acquire. Returns `Ok(false)` on timeout.
///
/// `IDXGIKeyedMutex::AcquireSync` reports timeout via a *success* HRESULT
...
fn acquire_keyed(mutex: &IDXGIKeyedMutex, timeout_ms: u32) -> anyhow::Result<bool> {
    ...
    // WAIT_ABANDONED still grants ownership (previous owner died).
    ...
    fn update_surface_from(
        ...
        match *self.texture.texture_for(width, height, format) {
            Some((ref surface, ref mutex)) => {
                if !acquire_keyed(mutex, MUTEX_TIMEOUT_MS)? {
                    // Game side is holding the mutex too long — drop this
                    // frame instead of blocking the host event loop.
                    return Ok(None);
                }
                defer!(unsafe {
                    _ = mutex.ReleaseSync(0);
            ...
            ref mut slot @ None => {
                ...
                // Freshly created texture: nothing else can hold its mutex yet.
                if !acquire_keyed(mutex, MUTEX_TIMEOUT_MS)? {
                    bail!("keyed mutex acquire timed out on freshly created texture");
                }
                defer!(unsafe {
                    _ = mutex.ReleaseSync(0);
                });
    ...
    pub fn update_bitmap(
        ...
        match *surface {
            Some((ref texture, ref mutex)) => {
                if !acquire_keyed(mutex, MUTEX_TIMEOUT_MS)? {
                    // Drop the frame rather than blocking the host event loop.
                    return Ok(None);
                }
                defer!(unsafe {
                    _ = mutex.ReleaseSync(0);
            ...
            None => {
                ...
                Ok(Some(UpdateSharedHandle {
                    handle: NonZeroU32::new(
                        unsafe { texture.cast::<IDXGIResource>()?.GetSharedHandle() }?.0 as u32,
                    ),
                }))
            }
        }
...
/// Perform an operation with an external texture, acquiring and releasing its
/// keyed mutex if available. Skips the operation (drops the frame) if the
/// mutex cannot be acquired within the bounded timeout — this must never
/// block or panic on the host main thread.
fn with_external_texture(
    texture: &ID3D11Texture2D,
    f: impl FnOnce(&ID3D11Texture2D) -> anyhow::Result<Option<UpdateSharedHandle>>,
) -> anyhow::Result<Option<UpdateSharedHandle>> {
