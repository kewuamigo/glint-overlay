...
pub enum WindowRequest {
    ...
    /// Set shared handle for a specific DXGI layer (`0` ≡ [`UpdateSharedHandle`]).
    UpdateLayerHandle(UpdateLayerHandle),

    /// Set position for a specific DXGI layer.
...
#[derive(Debug, Encode, Decode, Clone, PartialEq, Eq)]
/// Update a specific overlay DXGI layer surface.
pub struct UpdateLayerHandle {
    /// Layer index (`0` is the shell layer, same as [`UpdateSharedHandle`]).
    pub layer: u32,
    /// Shared texture handle; [`None`] removes the layer surface.
    pub handle: Option<NonZeroU32>,
}
impl_WindowRequestItem!(UpdateLayerHandle);

#[derive(Debug, Default, Encode, Decode, Clone, PartialEq)]
/// Set position of a specific overlay DXGI layer.
pub struct SetLayerPosition {
    /// Layer index.
    pub layer: u32,
    /// X position relative to the window client area.
    pub x: PercentLength,
    /// Y position relative to the window client area.
    pub y: PercentLength,
}
impl_WindowRequestItem!(SetLayerPosition);

#[derive(Debug, Default, Encode, Decode, Clone, PartialEq)]
/// Set hit-test rect for a DXGI layer. Draw still uses the full texture.
///
/// `x`/`y` are relative to the window client area (same as [`SetLayerPosition`]).
/// `width`/`height` are absolute pixels. When `rect` is [`None`], hit-test falls
/// back to the texture size at the layer position.
pub struct SetLayerInputRect {
    /// Layer index.
    pub layer: u32,
    /// Hit-test rect; [`None`] clears.
    pub rect: Option<LayerInputRect>,
}
impl_WindowRequestItem!(SetLayerInputRect);

#[derive(Debug, Default, Encode, Decode, Clone, PartialEq)]
/// Absolute-size hit-test rect with percent/length origin.
pub struct LayerInputRect {
    /// X origin relative to the window client area.
    pub x: PercentLength,
    /// Y origin relative to the window client area.
    pub y: PercentLength,
    /// Width in pixels.
    pub width: u32,
    /// Height in pixels.
