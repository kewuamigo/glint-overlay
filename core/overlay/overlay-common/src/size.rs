...
mod tests {
    ...
    #[test]
    fn resolve_percent_and_length() {
        assert!((PercentLength::Percent(0.5).resolve(200.0) - 100.0).abs() < f32::EPSILON);
        assert!((PercentLength::Length(42.0).resolve(999.0) - 42.0).abs() < f32::EPSILON);
        assert_eq!(PercentLength::ZERO.resolve(800.0), 0.0);
    }
}
