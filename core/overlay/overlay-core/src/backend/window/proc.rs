...
fn process_wnd_proc(
    ...
    match msg {
        msg::WM_WINDOWPOSCHANGED => {
            // Fails while the window is being destroyed; don't panic in the WndProc.
            ...
            } =>
        {
            ...
            if proc.input_blocking() {
                // Always ensure a visible cursor while blocking.  blocking_cursor
                // may temporarily be None if the Chromium renderer emitted a
                // cursor-changed:'none' event (e.g. right after startPainting
                // resumes following a stopPainting in Hidden mode, before the
                // first mouse-move reaches the renderer).  SetCursor(NULL) would
                // make the cursor invisible; fall back to the arrow cursor instead.
                let cursor = proc
                    .blocking_cursor
        ...
        msg::WM_MOUSEMOVE => {
            let mut proc = backend.proc.lock();
            if proc.listening_cursor() && !proc.input_blocking() {
...
pub(crate) unsafe extern "system" fn hooked_wnd_proc(
    ...
    // A panic here would abort the game process; the backend may already be
    // gone if a message arrives after WM_NCDESTROY cleanup.
...
fn cursor_input(id: u32, position: (i32, i32), lparam: LPARAM, event: CursorEvent) -> OverlayEvent {
    let [x, y] = bytemuck::cast::<_, [i16; 2]>(lparam.0 as u32);
    input_event::cursor_overlay_event(
        id,
        position,
        InputPosition {
            x: x as _,
...
fn keyboard_input(id: u32, input: KeyboardInput) -> OverlayEvent {
    input_event::keyboard_overlay_event(id, input)
}
